...
app = FastAPI(title="Power Control")
...
# def on_off(thing_model, status):
#     # turn on or off the pins in the raspberry pi
...
# the root will return html
@app.get("/")
async def root(req: Request):
    # response = db.query(ThingModel).all()
    return templates.TemplateResponse("index.html", {"request": req})


@app.get("/things/")
async def get_things(req: Request, db: session = Depends(get_db)):
...
async def create_thing(thing: Thing, db: session = Depends(get_db)):
    """
    Create a thing model and add to database
    """
...
async def update_thing(id: int, thing: Thing, db: session = Depends(get_db)):
    thing_model = db.query(ThingModel).filter(
        ThingModel.thing_id == id).first()  # check to see if the item exists
